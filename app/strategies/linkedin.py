    ...
    def apply(self, page, payload: ApplyPayload, dry_run: bool = True) -> ApplyResult:
        ...
        try:
            easy_apply_btn = page.locator(
                ...
                # Aria label patterns (English + Turkish + German etc.)
                "button[aria-label*='Easy Apply'], "
                "button[aria-label*='Kolay Başvuru'], "
                "button[aria-label*='Einfach bewerben'], "
                # Data attribute LinkedIn uses
                "button[data-control-name*='apply'], "
                # LinkedIn's apply button icon class
            ...
            return ApplyResult(False, "manual_required", "linkedin",
                               error_msg="Could not open Easy Apply modal (may not be an Easy Apply job).")

        # Step through the multi-page Easy Apply modal
        max_steps = 10
        for step in range(max_steps):
            _logger.info("   📝 Filling modal step %d...", step + 1)
            ...
            # Handle "Follow company" checkbox — uncheck it (language-agnostic via input[type=checkbox])
            try:
            ...
            # Navigation buttons — use aria-label which LinkedIn sets in the user's language
            # Fallback to footer button ordering (last button = primary action)
            footer_btns = page.locator(".jobs-easy-apply-modal footer button, [data-easy-apply-next-button]")
            ...
            if submit_btn.count() > 0 and submit_btn.is_visible():
                ...
                _logger.warning("   ⚠️  No navigation button found on step %d.", step + 1)
                break

        _logger.warning("   ⚠️  Reached max steps without submitting.")
        return ApplyResult(False, "manual_required", "linkedin",
                           error_msg="Reached max steps in Easy Apply modal.")
